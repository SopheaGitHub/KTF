...
if(isset($_POST) and $_SERVER['REQUEST_METHOD'] == "POST"){
	if(!empty($_POST['data'])){
		...
		if($_POST['data'] == 'cek'){
			...
			if(isset($_SESSION['user']) && !empty($_POST['receiver_id']) && !empty($_POST['date']) && !empty($_POST['tipe']) && isset($_POST['message']) && isset($_POST['images'])){
				$images = json_decode($_POST['images']);
				if(!empty($_POST['message']) && count($images) < 1){
					$data = $chat->send_message($_SESSION['user'], $_POST['receiver_id'], $_POST['message'], "", $_POST['date'], $_POST['tipe']);
					...
					foreach($images as $image){
						...
						if($h == 0){
							$data = $chat->send_message($_SESSION['user'], $_POST['receiver_id'], $_POST['message'], $image->name, $_POST['date'], $_POST['tipe']);
						}else{	
							$data = $chat->send_message($_SESSION['user'], $_POST['receiver_id'], "", $image->name, $_POST['date'], $_POST['tipe']);
						}
						$h++;
					}
				}else if(empty($_POST['message']) && count($images) > 0){
