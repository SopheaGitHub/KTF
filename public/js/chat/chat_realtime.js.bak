...
var chat_realtime = function(j, k, l, m, n, imageDir) {
        ...
        if (a.val().tipe == 'login') {
            if (a.val().name != m) {
                if ($('#' + a.val().name).length) {
                    ...
                    var b = '<li id="' + a.val().name + '" class="user bounceInDown" data-tipe="users" data-uid="' + a.ke + '" data-uname="' + a.name + '">';
    ...
    //send chat
    $(document).on('click', '.sendchat', function(h){
        var a = new Date(),
            b = a.getDate(),
            c = (a.getMonth() + 1),
            d = a.getFullYear(),
    ...
    function userMysql() {
        $.ajax({
            ...
            success: function(a) {
                ...
                $.each(a, function(i, a) {
                    if (a.name != m) {
                        b += '<li id="' + a.name + '" class="user bounceInDown" data-tipe="users" data-uid="' + a.ke + '" data-uname="' + a.name + '">';
                        b += '	<a href="#" class="clearfix">';
